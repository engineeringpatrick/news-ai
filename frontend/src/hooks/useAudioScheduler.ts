...
import {apiAppendTranscript, apiNextItems, apiRenderItem} from '../api';
import type {
	DialogueLine,
	NewsStory,
	PersonaConfig,
...
export function useAudioScheduler({
	...
	const [nowPlaying, setNowPlaying] = useState<RenderedItemWithPersona | null>(
		null,
	);
	...
	useEffect(() => {
		...
		const MinStories = 10;
		if (queue.length >= MinStories) return;
	...
	useEffect(() => {
		...
		const MaxRenderedAhead = 5;
		if (renderedQueue.length >= MaxRenderedAhead) return;
		...
		(async () => {
			try {
				...
				const rendered = await apiRenderItem(
					queue[storyToRenderIdx] as NewsStory,
					personaConfig,
				);
				if (!rendered) {
					return;
